...
class LWN(nn.Module):
    def __init__(self, dim, wavelet='haar', initialize=True, head=4, drop_rate=0., use_ca=False, use_sa=False):
        ...
        self.conv_high_freq = nn.Conv2d(dim*4, dim*4, kernel_size=3, padding=1)
        ...
        self.esdb = ESDB(in_channels=dim, out_channels=dim, conv=BSConvU)
        self.fmblock_h = FMBlock(dim, kernel_size=7, mlp_ratio=1.25, conv_ratio=1.5)
        self.fmblock_v = FMBlock(dim, kernel_size=7, mlp_ratio=1.25, conv_ratio=1.5)
        self.fmblock_d = FMBlock(dim, kernel_size=7, mlp_ratio=1.25, conv_ratio=1.5)
    ...
    def forward(self, x):
        ...
        ya, (yh, yv, yd) = self.wavedec(x)   #DWT

        ya = self.conv1(ya)
        ya = self.conv2(ya)
        ya = self.conv3(ya)

        yh = self.fmblock_h(yh)  # Enhance horizontal high frequency
        yv = self.fmblock_v(yv)  # Enhance vertical high frequency
        yd = self.fmblock_d(yd)
